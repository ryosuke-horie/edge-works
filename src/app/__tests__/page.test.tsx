...
describe("Home", () => {
	it("正しくレンダリングされること", () => {
		...
		const catchPhrase = screen.getAllByText(
			/月額|3,000|円|で|ホームページを|制作/,
		);
		...
		// フッターが表示されることを確認
		expect(
			screen.getByText("個人事業主のためのウェブサイト制作サービス"),
		).toBeTruthy();
	});
	...
	it("FAQセクションが正しく表示されること", () => {
		...
		// FAQのheading要素を特定
		const faqHeadings = screen.getAllByRole("heading", {
			name: "よくある質問",
		});
	...
	it("料金プランの内容が正しく表示されること", () => {
		...
		// 料金プランのheading要素を特定
		const pricingHeadings = screen.getAllByRole("heading", {
			name: "シンプルな料金プラン",
		});
